...
const allQuestions = [
  ...
  },
  {
    ...
    year: "平成27年 第2問",
    title: "WEBサイトの開発",
    text: "自社のWebサイトの開発にあたっては、利用可能な様々な言語や仕組みがあり、Webコンテンツごとに必要な機能や表現に合ったものを使用する必要がある。これらの言語や仕組みの特徴に関する以下の(1)～(4)の記述と、その名称の組み合わせとして、最も適切なものを選べ。\n\n(1) Webページに記述された文書・データの表示位置の指示や表の定義、および、文字修飾指示等の表示方法に関する事項を記述するもの。\n(2) Webページ内でHTMLとともに記述することができるスクリプト言語で、サーバ側においてスクリプトを処理し、その結果を端末側で表示することが可能であり、データベースとの連携も容易である。\n(3) Webページの中に実行可能なコマンドを埋め込み、それをサーバ側で実行させ、実行結 果を端末側で表示させる仕組み。\n(4) コンピュータグラフィックスに関する図形、画像データを扱うベクターイメージデータをXMLの規格に従って記述するもの。",
    options: [
      "(1): CSS   (2): ASP   (3): PHP   (4): SGML",
      "(1): CSS   (2): PHP   (3): SSI   (4): SVG",
      "(1): SMIL  (2): Javaアプレット  (3): ASP   (4): SSI",
      "(1): SVG   (2): SMIL  (3): PHP   (4): SGML"
    ],
    answerIndex: 1,
    explanation: `解答: イ\n\nWebサイトの開発にあたって利用可能な様々な言語や仕組みについて細かな知識を確認する問題です。\n\n・記述(1)は、CSS（Cascading Style Sheets）です。HTMLのプログラムが煩雑になるのを防ぎ、デザインの組み合わせをあらかじめ定義しておくことができます。\n・記述(2)は、PHP（Hypertext Preprocessor）です。動的サービスを作成するための言語で、データベースを操作する命令が組み込まれているのが特徴です。\n・記述(3)は、SSI（Server Side Include）です。HTMLの中にWebサーバ側で実行するコマンドを埋め込んでおき、その実行結果をクライアントサーバに返します。\n・記述(4)は、SVG（Scalable Vector Graphics）です。XMLをベースとした、2次元ベクターイメージ用の画像形式です。\n\n以上より、選択肢イが適切であり、これが正解です。`,
    table: null
  },
  {
    id: 6,
  ...
  },
  {
    ...
    text: "Web環境におけるソフトウェア開発においては、開発目的に応じて利用可能なさまざまなプログラミング言語などを組み合わせて実現していくことが必要になる。以下の(1)～(4)の記述と、それらに対応するプログラミング言語などの組み合わせとして、最も適切なものを下記の解答群から選べ。\n\n(1) HTMLファイルの中で記述され、動的なWebページを作成することができる。\n(2) データベースと連携したWebページを作成することができる。\n(3) Webサーバと非同期通信を行うことで、Webページの一部分のみのデータ内容を動的に更新することができる技術である。\n(4) Webページのフォントや文字の大きさ、行間、表示位置の指示など、表示方法に関する事項を定義するために利用する",
    options: [
      "(1): Java (2): jQuery (3): Perl (4): CSS",
      "(1): Java (2): PHP (3): Perl (4): XSL",
      "(1): JavaScript (2): jQuery (3): Ajax (4): XSL",
      "(1): JavaScript (2): PHP (3): Ajax (4): CSS"
    ],
    answerIndex: 3,
    explanation: `解答: エ\n\n・記述(1)は、JavaScriptです。Webページに動きや機能を埋め込むためのスクリプト言語です。\n・記述(2)は、PHP（Hypertext Preprocessor）です。データベースとの連携が容易です。\n・記述(3)は、Ajaxです。Webサーバと非同期通信を行う技術であり、画面の一部のみ動的に再描画できます。\n・記述(4)は、CSS（Cascading Style Sheets）です。表示方法に関する事項を定義します。\n\n以上より、(1)がJavaScript、(2)がPHP、(3)がAjax、(4)がCSSの選択肢エが適切であり、これが正解です。`,
    table: null
  }
];
